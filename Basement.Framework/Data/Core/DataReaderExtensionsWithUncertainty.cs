...
{
    public static class DataReaderExtensionsWithUncertainty
    {
        ...
        public static IEnumerable<T> ToListWithUncertainty<T>(this IDataReader reader)
        {
        ...
        public static class EntityConverter<T>
        {
            ...
            private static void _init(IDataReader reader)
            {
                 //batchDataLoader = CreateBatchDataLoader(Entity<T>.EntityProperties, reader);
                batchDataLoader = CreateBatchDataLoader(reader);
            }

            //private static Converter<IDataReader, IEnumerable<T>> CreateBatchDataLoader(IList<EntityProperty> columnInfoes, IDataReader reader)
            private static Converter<IDataReader, IEnumerable<T>> CreateBatchDataLoader(IDataReader reader)
            {
                ...
                EntityProperty[] columnInfoes = null;
                LocalBuilder[] colIndices = GetColumnIndices(il, reader, out columnInfoes);
                il.MarkLabel(loop);
            ...
            //private static LocalBuilder[] GetColumnIndices(ILGenerator il, IDataReader reader, out EntityProperty[] columnInfos)
            //{
            //    int fieldcount = reader.FieldCount;

            //    columnInfos = new EntityProperty[fieldcount];
            //    LocalBuilder[] colIndices = new LocalBuilder[fieldcount];
            //    for (int i = 0; i < fieldcount; i++)
            //    {
                    
            //        columnInfos[i] = Entity<T>.EntityProperties.First(s => s.ColumnName.Equals(reader.GetName(i), StringComparison.OrdinalIgnoreCase));
            ...
            private static LocalBuilder[] GetColumnIndices(ILGenerator il, IDataReader reader, out EntityProperty[] columnInfos)
            {
                List<EntityProperty> entitys = new List<EntityProperty>();
                for (int i = 0; i < reader.FieldCount; i++)
                {
                    if (Entity<T>.EntityProperties.Count(s => s.ColumnName.Equals(reader.GetName(i), StringComparison.OrdinalIgnoreCase)) == 1)
                    {
                        entitys.Add(Entity<T>.EntityProperties.First(s => s.ColumnName.Equals(reader.GetName(i), StringComparison.OrdinalIgnoreCase)));
                    }
                }

                columnInfos = entitys.ToArray();
                LocalBuilder[] colIndices = new LocalBuilder[columnInfos.Length];
                ...
                for (int i = 0; i < columnInfos.Length; i++)
                {
                    ...
                    colIndices[i] = il.DeclareLocal(typeof(int));
                    il.Emit(OpCodes.Ldarg_0);
                    il.Emit(OpCodes.Ldstr, columnInfos[i].ColumnName);
                    il.Emit(OpCodes.Callvirt, DataRecord_GetOrdinal);
                    il.Emit(OpCodes.Stloc_S, colIndices[i]);
                }
                return colIndices;
            ...
            private static void BuildItem(ILGenerator il, EntityProperty[] columnInfoes, LocalBuilder item, LocalBuilder[] colIndices)
            {
